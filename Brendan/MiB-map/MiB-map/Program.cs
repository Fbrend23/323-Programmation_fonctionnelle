...
// Nouvelle liste qui contiendra les produits anonymisés
var anonymousProducts = products.Select(p => new Product
{ 
    Producer = p.Producer.Length > 3
                   ? p.Producer.Substring(0, 3) + "..." + p.Producer[^1]
                   : p.Producer + "...",    
    ProductName = p.ProductName,
    MaxCA = products
            .Select(x => x.ProductName == p.ProductName
                         ? x.Quantity * x.PricePerUnit
                         : 0)
            .Max()
}).ToList();

anonymousProducts.ForEach(p =>
    Console.WriteLine($"{p.Producer} | {p.ProductName} | {p.MaxCA}")
);
