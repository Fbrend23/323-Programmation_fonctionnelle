...
string pathA = "C:/Users/pl76tup/Desktop/323-Programmation_fonctionnelle/exos/diffit/v2.txt";

Console.Write("Fichier B: ");
//string? pathB = Console.ReadLine();
string pathB = "C:/Users/pl76tup/Desktop/323-Programmation_fonctionnelle/exos/diffit/v2.txt";
...
Func<string, string> enforceCase = text => text.ToUpperInvariant();
...
{
    comparisons.Add(new LinesComparison
    {
        Number = i,
        ContentA = linesA[i],
        ContentB = linesB[i]
...
int sameCount = comparisons.Count - diffLines.Count;
Console.WriteLine($"> Résumé : {sameCount} ligne(s) identique(s), {diffLines.Count} différente(s).");
...
{
    int minLen = Math.Min(c.ContentA.Length, c.ContentB.Length);
    int diffs = 0;
    for (int i = 0; i < minLen; i++)
        if (c.ContentA[i] != c.ContentB[i]) diffs++;
    return diffs + Math.Abs(c.ContentA.Length - c.ContentB.Length);
};
...
if (diffLines.Count > 0)
{
    ...
    {
        Console.WriteLine($"\nLigne {c.NumberHuman} : {countVariations(c)} variation(s)");
        PrintColoredDiff(c);
    }
    /// Diff coloré
    ...
    void PrintColoredDiff(LinesComparison c)
    {
        int minLen = Math.Min(c.ContentA.Length, c.ContentB.Length);
